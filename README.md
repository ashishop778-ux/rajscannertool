...
Rajscantool can be easily installed using `pip`.

```bash
pip install Rajscantool
```

## Usage

After installation, simply run the tool from your terminal:

```bash
Rajscantool
```

This will launch the main menu, where you can select the desired scanning or extraction option.
...
1.  **Run the tool:**
    ...
    Rajscantool
    ```
2.  **Select an option** (e.g., `1` for Host Scanner).
3.  **Follow the prompts** to provide necessary inputs like domain lists, CIDR ranges, or text.
4.  **View results** directly in the terminal and in the generated output files.

## Dependencies

Rajscantool relies on the following Python libraries, which will be automatically installed with `pip`:

*   `requests`: For making HTTP requests during scanning.
*   `colorama`: For cross-platform colored terminal output.
*   `rich`: For beautiful and informative terminal rendering (progress bars, tables, panels).
*   `urllib3`: A powerful, user-friendly HTTP client for Python.

## Developer

*   **Name**: Mr. Raj
*   **Alias**: Mr. Tech Hacker
*   **GitHub**: [RajownerTech](https://github.com/RajownerTech)
*   **YouTube**: Mr Tech Hacker
*   **Instagram**: @raj_dark_official
*   **Telegram**: Mr Tech Hacker

## License

This project is licensed under the MIT License - see the `LICENSE` file for details. <!-- Placeholder for LICENSE file -->
